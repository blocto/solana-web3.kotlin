...
class Transaction {
     ...
     */
    fun partialSign(vararg signers: Signer) {
        require(signers.isNotEmpty()) { "No signers" }
    ...
    private fun partialSign(message: Message, signers: List<Signer>) {
     ...
     */
    suspend fun getEstimatedFee(connection: Connection): Long {
        return (connection.getFeeForMessage(this.compileMessage())).value
    }
